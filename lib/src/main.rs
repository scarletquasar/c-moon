	...
	unsafe {
		...
		match args[1].as_str() {
			"io" => {
				...
				let result = io_methods::execute_from_json(&args[2].clone());
				println!("[io] Command executed with success, result: {}", result);
			}
			"api" => {
				println!("[api] Executing function with argument: {}", args[2]);
				let result = api_methods::execute_from_json(&args[2].clone()).await;
				println!("[api] Command executed with success, result: {}", result.unwrap_err_unchecked());
			}
			"download-youtube" => {
